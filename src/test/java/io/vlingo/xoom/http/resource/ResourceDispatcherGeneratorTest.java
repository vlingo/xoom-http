...
import io.vlingo.xoom.http.resource.ResourceDispatcherGenerator.Result;
...
public class ResourceDispatcherGeneratorTest {
  ...
  public void setUp() {
    actionPostUser = new Action(0, "POST", "/users", "register(body:io.vlingo.xoom.http.sample.user.UserData userData)", null);
    actionPatchUserContact = new Action(1, "PATCH", "/users/{userId}/contact", "changeContact(String userId, body:io.vlingo.xoom.http.sample.user.ContactData contactData)", null);
    actionPatchUserName = new Action(2, "PATCH", "/users/{userId}/name", "changeName(String userId, body:io.vlingo.xoom.http.sample.user.NameData nameData)", null);
    ...
    try {
      resourceHandlerClass = (Class<? extends ResourceHandler>) Class.forName("io.vlingo.xoom.http.sample.user.UserResource");
    } catch (Exception e) {
      resourceHandlerClass = ConfigurationResource.newResourceHandlerClassFor("io.vlingo.xoom.http.sample.user.UserResource");
    }
